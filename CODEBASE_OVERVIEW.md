...
- **Message stream (center)**: on selecting a topic, loads its most recent messages (default limit 50, capped at 500) merged and sorted newest-first across *all* partitions
  ...
  - "Purge messages" deletes all records in every partition (topic/partitions remain); "Delete topic" removes the topic entirely (requires typing the topic name to confirm)
